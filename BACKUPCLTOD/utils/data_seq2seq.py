    ...
    for dial in data:
        ...
        for idx_t, t in enumerate(dial['dialogue']):
            ...
            if(t['spk']=="USER"):
                ...
                slots = t["utt"].strip()
                # for inte in t["service"]:
                #     slots = slots.replace(inte,"")
                dialogues.append({"history":" ".join(plain_history[-args.max_history:]),
...
def update_task_id(args,dataset_dic, new_task_id):
    new_dataset_dic = []
    for d in dataset_dic:
        d["task_id"] = new_task_id
        new_dataset_dic.append(d)
    return new_dataset_dic
...
def make_loader(args,list_sample,tokenizer):
    ...
    return DataLoader(DatasetTrain(list_sample), batch_size=args.train_batch_size, shuffle=True,collate_fn=partial(collate_fn_, tokenizer=tokenizer),num_workers=16)
...
def get_data_loaders(args, tokenizer, test=False):
    ...
    if(args.continual):
        if(not test):
            for task_id, dataset_task in datasets["train"].items():
                if(task_id in common_task_id):
                    train_loaders[task_id] = DataLoader(DatasetTrain(dataset_task), batch_size=args.train_batch_size, shuffle=True,collate_fn=partial(collate_fn_, tokenizer=tokenizer),num_workers=16)
                    train_datasets[task_id] = dataset_task
            for task_id, dataset_task in datasets["dev"].items():
                if(task_id in common_task_id):
                    valid_loaders[task_id] = DataLoader(DatasetTrain(dataset_task), batch_size=args.valid_batch_size, shuffle=False,collate_fn=partial(collate_fn_, tokenizer=tokenizer),num_workers=16)
        ...
        if(not test):
            ...
            train_loaders = DataLoader(DatasetTrain(dataset_train), batch_size=args.train_batch_size, shuffle=True,collate_fn=partial(collate_fn_, tokenizer=tokenizer),num_workers=16)

            dataset_dev = []
            for task_id, dataset_task in datasets["dev"].items():
                if(task_id in common_task_id):
                    dataset_dev += dataset_task
            valid_loaders = DataLoader(DatasetTrain(dataset_dev), batch_size=args.valid_batch_size, shuffle=False,collate_fn=partial(collate_fn_, tokenizer=tokenizer),num_workers=16)
    ...
    test_loaders = DataLoader(DatasetTrain(sum(temp_list,[])), batch_size=args.valid_batch_size, shuffle=False,collate_fn=partial(collate_fn_, tokenizer=tokenizer),num_workers=16)
