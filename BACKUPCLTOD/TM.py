...
def loadCSV(split):
    split_id = []
    with open(f'data/Taskmaster/TM-1-2019/train-dev-test/{split}.csv') as f:
        for l in f:
...
def preprocessTM2020(develop=False):
    ...
    # train_data, dev_data, test_data = np.split(data, [int(len(data)*0.8), int(len(data)*0.9)])
    # print(len(train_data.tolist()), len(dev_data.tolist()), len(test_data.tolist()))
    # return train_data.tolist(), dev_data.tolist(), test_data.tolist()


### To split self dialogue
# split_id_train, split_id_dev, split_id_test = loadCSV("train"), loadCSV("dev"), loadCSV("test")

# train_data, dev_data, test_data = [], [], []
# for dial in data:
#     # print(dial["id"])
#     if dial["id"] in split_id_train:
#        train_data.append(dial)
#     if dial["id"] in split_id_dev:
#        dev_data.append(dial)
#     if dial["id"] in split_id_test:
