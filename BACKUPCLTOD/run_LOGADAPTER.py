...
if __name__ == '__main__':
    ...
    parser.add_argument("--valid_batch_size", type=int, default=25, help="Batch size for validation")
    ...
    parser.add_argument("--CL", type=str, default="LOGADAPTER", help="CL strategy used")
    ...
    # parser.add_argument("--episodic_mem_size", type=int, default=100, help="number of batch put in the episodic memory")
    ...
    parser.add_argument("--number_of_adpt", type=int, default=20, help="lenght of the generation")
    parser.add_argument("--load_check_point", type=str, default="None", help="")
    parser.add_argument("--current_task_to_load", type=int, default=-1, help="lenght of the generation")
    ...
    parser.opt_list('--task_type', type=str, default="E2E", options=["NLG"], tunable=True)
    # parser.opt_list('--task_type', type=str, default="E2E", options=["E2E"], tunable=True)
    parser.opt_list("--bottleneck_size", type=int, default=300,options=[50, 75, 100], tunable=True)
    parser.opt_list('--episodic_mem_size', default=1, type=int, options=[25,50,100], tunable=True)
    parser.opt_list("--lr", type=float, default=6.25e-5, options=[6.25e-3], tunable=True)
    # parser.opt_list("--lr", type=float, default=6.25e-5, options=[0.00230449368], tunable=True)
    
    parser.opt_list('--n_epochs', default=10, type=int, options=[10], tunable=True)
    # parser.opt_list('--seed', default=1, type=int, options=[1], tunable=True)
    parser.opt_list('--seed', default=1, type=int, options=[1,2,3,4,5], tunable=True)
    parser.opt_list('--compressing_factor', default=1, type=int, options=[2], tunable=True)
