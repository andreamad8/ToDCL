...
# from utils.data import get_data_loaders, add_special_tokens_, MODEL_INPUTS,SPECIAL_TOKENS
from utils.parser import get_args,set_global_logging_level
from collections import defaultdict
from utils.data_seq2seq import make_loader
from utils.data_seq2seq import get_data_loaders, get_current_task_data, make_loader


set_global_logging_level(logging.ERROR)

def generate(args,model,tokenizer,input_ids,token_type_ids):
    output_sequences = model.generate(
        input_ids=input_ids,
        token_type_ids=token_type_ids,
        max_length=args.length + len(input_ids[0]),
        # min_length= 2 + len(input_ids[0])
    ...
    for _, generated_sequence in enumerate(output_sequences):
        ...
        generated_answers.append(text[len(tokenizer.decode(input_ids[0], clean_up_tokenization_spaces=True)) :])
    return generated_answers[0]#[:generated_answers[0].find("<eos>")].strip()
...
def test_model_seq2seq_ADAPTER(args,model,tokenizer,test_loader,test_dataset,time="0_['']",max_seen_task=0):
    ...
    model.model.eval()
    results = []
    if(args.CL=="LOGADAPTER"):
        print(max_seen_task)
        range_adpt = max_seen_task+1
    else:
        print(model.task_list_seen,len(model.task_list_seen))
        range_adpt = len(model.task_list_seen)
    # if args.supsup:
    #     for idx_b, batch in tqdm(enumerate(test_loader),total=len(test_loader)):
    #         task_ids_pred = model.compute_task_SupSup(batch,device=device) ## one value per batch
    #         for (d_id, t_id, ta_id, task_id_pred) in zip(batch["dial_id"],batch["turn_id"],batch["task_id"],task_ids_pred):
    #             perplexity_dict[f'{d_id}_{t_id}_{ta_id}'] =  int(task_id_pred)
    # else:
