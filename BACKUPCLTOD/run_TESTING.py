...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_batch_size", type=int, default=14, help="Batch size for training")
    ...
    parser.add_argument("--CL", type=str, default="ADAPTER", help="CL strategy used")
    ...
    parser.add_argument("--number_of_adpt", type=int, default=40, help="lenght of the generation")
    ...
    # parser.opt_list('--task_type', type=str, default="E2E", options=["INTENT","NLG","DST"], tunable=True)
    parser.opt_list('--task_type', type=str, default="E2E", options=["INTENT"], tunable=True)
    parser.opt_list("--bottleneck_size", type=int, default=300,options=[50], tunable=True)
    parser.opt_list("--permutation", type=int, default=0,options=[1], tunable=True)
    parser.opt_list("--lr", type=float, default=6.25e-5, options=[6.25e-3], tunable=True)
    parser.opt_list('--n_epochs', default=10, type=int, options=[1], tunable=True)
    ...
    # #SBATCH --cmd=value
    # ############
    cluster.add_slurm_cmd(cmd='cpus-per-task', value='20', comment='CPUS per task.')
    cluster.add_slurm_cmd(cmd='mem', value='0', comment='Memory')
    cluster.add_slurm_cmd(cmd='constraint', value='volta16gb', comment='GPU type per task.')
    cluster.add_slurm_cmd(cmd='time', value='0:40:0', comment='GPU type per task.')
